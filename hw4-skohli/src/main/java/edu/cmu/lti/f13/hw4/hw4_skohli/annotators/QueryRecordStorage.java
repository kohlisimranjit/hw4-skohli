...
public class QueryRecordStorage extends JCasAnnotator_ImplBase {
	...
	private void createTermFreqVector(JCas jcas, Document doc) {
		...
		String relString = docText.substring(relPatternMatcher.start() + 4,
				relPatternMatcher.end());
		String qidString = docText.substring(qidMatcher.start() + 4,
				qidMatcher.end());
		...
		doc.setText(docText.substring(relPatternMatcher.end(),
				docText.length() - 1).trim());
		// TO DO: construct a vector of tokens and update the tokenList in CAS

		/*
		 * FSArray fsArray = null; // new FSArray(jcas, 10);
		 * 
		 * String docText = doc.getText(); String jcasString =
		 * jcas.getDocumentText(); String jcasSentence[] =
		 * jcasString.split("\n"); String docTextSentence[] =
		 * docText.split("\n");
		 * 
		 * // int j = 0;
		 * 
		 * QueryResultGroup queryGroup = null; for(int i=0;i<10;i++)
		 * {System.out.println(i);} for (int i=0;i<jcasSentence.length;i++) {
		 * String sentence = jcasSentence[i];
		 * System.out.println(i+"->now checking==========" + sentence); Matcher
		 * qidMatcher = qidPattern.matcher(sentence); Matcher relPatternMatcher
		 * = relPattern.matcher(sentence);
		 * 
		 * EnrichedDocument enrichedDocument = new EnrichedDocument(jcas);
		 * 
		 * enrichedDocument.setText(docTextSentence[i]); qidMatcher.find();
		 * 
		 * relPatternMatcher.find(); if (sentence.indexOf("rel=99") > -1) { //j
		 * = 0; queryGroup = new QueryResultGroup(jcas);
		 * 
		 * enrichedDocument.setRelevanceValue(99);
		 * queryGroup.setQuery(enrichedDocument); fsArray = new FSArray(jcas,
		 * 10); System.out.println("fsArray->" + fsArray);
		 * queryGroup.setResults(fsArray); } else {
		 * System.out.println("fsArray->" + fsArray); //fsArray.set(j++,
		 * enrichedDocument);
		 * 
		 * }
		 * 
